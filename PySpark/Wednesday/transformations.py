...
levels = logs.map(lambda line: line.split()[2]) #(hint: split and take index 2)
...
sorted_counts = word_counts.sortBy(lambda w: w[1], ascending=False)
...
parsed = web_logs.map(parse_log)

successful = parsed.filter(lambda r: r["status"] == 200)
endpoints = successful.map(lambda p: p["path"])
request_count = endpoints.map(lambda e: (e, 1)).reduceByKey(lambda a, b: a + b)
sorted_by_count = request_count.sortBy(lambda n: n[1], ascending=False)

print(f"Requests per endpoint: {sorted_by_count.collect()}")
