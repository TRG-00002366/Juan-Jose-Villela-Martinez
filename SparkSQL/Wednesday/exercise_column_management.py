...
inventory.na.fill({'warehouse':'CENTRAL'}).show()

# TODO 2b: Replace null quantity with 0
inventory.na.fill({'quantity':0}).show()

# TODO 2c: Create an "in_stock" boolean column (quantity > 0 or not null)
inventory.withColumn(
    'in_stock',
    (col('quantity') > 0) | col('quantity').isNotNull()
).show()
...
descriptions.select(
    split(col('description'), ' - ').getItem(0).alias('product_name')
).show()

# TODO 6b: Extract the size value
descriptions.select(
    trim(split(split(col('description'), ',').getItem(0), ':').getItem(1)).alias('size')
).show()

# TODO 6c: Extract the color value
descriptions.select(
    trim(split(split(col('description'), ',').getItem(1), ':').getItem(1)).alias('color')
).show()
